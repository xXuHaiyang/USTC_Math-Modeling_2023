...
def plot_matches(image0,
    ...
    # normalize the (0, 0) to be the center of the image
    if write_file is not None:
        ...
        fmkpts = np.concatenate([fmkpts0, fmkpts1], axis=1) # shape (N, 4)
        fmkpts = fmkpts.T # shape (4, N)
        
        print("write file: ", write_file)
        with open(write_file, 'w') as f:
            for i in range(fmkpts.shape[0]):
                f.write(' '.join([str(x) for x in fmkpts[i, :]]) + '\n')
...
if __name__ == '__main__':
    ...
    prefix = args.input.split('/')[-2]
    if not os.path.exists("./"+prefix):
        os.mkdir("./"+prefix)
    
    for i in range(0, len(image_loader)-1):
        
        img_ref = image_loader[i]
        ...
        vis_img = plot_matches(img_ref, img, kpts_ref, kpts, matches, 
                               write_file="./"+prefix+"/pair{}{}.txt".format(i+1, i+2))
        ...
        cv2.imwrite("./"+prefix+"/pair{}{}.png".format(i+1, i+2), vis_img)
